...
let name = addProductForm["productName"];
let category = addProductForm["productCategory"];
let price = addProductForm["productPrice"];
let quantity = addProductForm["productQuantity"];
let measurement = addProductForm["productMeasurement"];
let description = addProductForm["productDescription"];
// let address = addProductForm["productAddress"];
// let state = addProductForm["productState"];
// let city = addProductForm["productCity"];
let frontView = addProductForm["productFrontView"];
let backView = addProductForm["productBackView"];
let leftView = addProductForm["productLeftView"];
let rightView = addProductForm["productRightView"];
...
function validateCategory(cat) {
    cat = cat.value.trim();
    if (cat === "") {
        errors.name = "The product category field is required.";
    } else if (typeof cat !== "string") {
        errors.productCategory = "The product category must be a string.";
...
function validateMeasurement(meas) {
    ...
    if (meas === "") {
        ...
        errors.measurement = "The product measurement must not be greater than 50 characters.";
    } else {
        errors.measurement = "";
    }
...
function validateFrontView(img) {
    ...
    if (img === "") {
        errors.frontView = "The product front view field is required.";
    } else if (
...
function validateProduct() {
    ...
    axios.post("{{route('users.admin.products.store')}}", formData)
    .then(function (response) {
        ...
        if(data){
            ...
            if(data.productSuccess){
                ...
                setTimeout(() => {
                    window.location.assign("{{route('users.admin.products')}}");
                }, 2000);
            }
        }
    })
