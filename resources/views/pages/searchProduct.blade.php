...
@include('header')
    ...
    <div class="container">
      <nav aria-label="breadcrumb">
        <ol class="breadcrumb">
          ...
          <li class="breadcrumb-item active" aria-current="page">Search / Sort Result</li>
        </ol>
      </nav>
    </div>
  </div>
  ...
  <section class="our-products-area section-padding-50">
    <div class="container">
      <div class="row">
        <div class="col-12">
          <!-- Section Heading -->
          <div class="section-heading text-center">
            <h2><span>{{$products->count()}} of {{$products->total()}} </span> Search Result<?php echo $products->total() > 1 ? 's': ""; ?></h2>
...
<div class="all-products-page-container">
  <div class="products-category-filter">
      <p class="filter-title mt-5"> Categories </p>
        <ul>
          <form action="{{route('pages.searchProductCategory')}}" method="get">
            @foreach ($categories as $category)
              @if ($category->name === 'Others')
                  @continue
              @endif
              <li><input type="submit" class="mb-2 border-0 bg-white" name="category" value="{{$category->name}}"></li>
            @endforeach
              <li><input type="submit" class="border-0 bg-white" name="category" value="Others"></li>
          </form>
        </ul>
  </div>
